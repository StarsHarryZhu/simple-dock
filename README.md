...
- `设置 → 通用设置` 出现六行：底栏统计坞（开关）/ 底栏面板样式 / 价格表 / 成本计价币种 / 面板玻璃 / 背景色
...
收起时为一行三个按钮：**最左 `步数 N`**，**右侧 `命中率 X%` 与 `上下文 Y%`**（两组分开）。三个按钮外观一致（**无箭头、收起态无背景**），点任意一个展开对应面板，**同一时刻只开一个**（点其它按钮或面板外、按 Esc 即关闭）。上下文不再使用 DSH 官方指示器——本插件用同一份 token-meter 数据自渲染，因此面板背景与另外两个完全一致（传统实底 / 半透明玻璃，跟随模糊、磨砂与背景色设置）。
...
| **上下文面板**：占用百分比 ＋ `~已用 / 上限` ＋ 系统提示 / 工具 / 消息的近似分解 | — |

## 设置（`设置 → 通用设置`）

| 说明 | 效果 |
|---|---|
| **底栏统计坞**：总开关，关闭后立即恢复 DSH 官方底栏（停用时这一行仍在，方便再开回来）<br>**底栏面板样式**：半透明（玻璃）/ 传统（实底）<br>**面板玻璃**：模糊度 0–40 px、磨砂度 0–100% 滑杆，拖动实时生效<br>**背景色**：半透明模式与传统模式各一个取色器（可「跟随主题」恢复默认），面板与按钮背景实时更新<br>**成本计价币种**：美元 USD / 人民币 CNY<br>**价格表**：内置（v4-pro / v4-flash 峰谷计价，不联网） | <img src="assets/thumbs/04-settings.png" alt="设置项" width="480" /> |
...
- 插件开关（三处同一状态：`设置 → 通用设置` 第一行、`设置 → 插件` 的 Simple Dock 标签页、侧栏**插件**页里本 bundle 的配置）：关闭后**立即恢复 DSH 官方底栏**（本插件的底栏注册随之注销），重新开启即恢复统计坞，**不需要刷新或重启**
