...
- 底栏出现 `步数 N`（最左）｜ `命中率 X%` + 官方上下文指示器（右侧）
- `设置 → 插件` 出现 **Simple Dock** 标签页（开关在里面）；侧栏 **插件** 页里本 bundle 的配置处也有同一个开关
- `设置 → 通用设置` 出现五行：底栏统计坞（开关）/ 底栏面板样式 / 价格表 / 成本计价币种 / 面板玻璃
...
收起时为一行：**最左 `步数 N`**，**右侧 `命中率 X%`**，再右边是 DSH 官方的**上下文指示器**（圆环 + 占用百分比，点击展开官方上下文明细）；点击左右区域分别展开本插件的明细面板。上下文胶囊的背景跟随本插件的「底栏面板样式」与「面板玻璃」设置（半透明时是玻璃、传统时是实底，模糊/磨砂滑杆实时生效），因此与我们的胶囊观感一致。
...
| 说明 | 效果 |
|---|---|
| **底栏统计坞**：总开关，关闭后立即恢复 DSH 官方底栏（停用时这一行仍在，方便再开回来）<br>**底栏面板样式**：半透明（玻璃）/ 传统（实底）<br>**面板玻璃**：模糊度 0–40 px、磨砂度 0–100% 滑杆，拖动实时生效（上下文指示器的背景也跟随）<br>**成本计价币种**：美元 USD / 人民币 CNY<br>**价格表**：内置（v4-pro / v4-flash 峰谷计价，不联网） | <img src="assets/thumbs/04-settings.png" alt="设置项" width="480" /> |
