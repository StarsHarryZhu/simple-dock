...
| 说明 | 效果 |
|---|---|
| **底栏统计坞**：总开关，关闭后立即恢复 DSH 官方底栏（停用时这一行仍在，方便再开回来）<br>**底栏面板样式**：半透明（玻璃）/ 传统（实底）<br>**面板玻璃**：模糊度 0–40 px、磨砂度 0–100% 滑杆，拖动实时生效<br>**背景色**：两个取色窗格 —— 半透明模式色、传统模式色（不自定义时显示主题默认色，选色后即固定使用），面板与展开内容实时更新<br>**成本计价币种**：美元 USD / 人民币 CNY<br>**价格表**：内置（v4-pro / v4-flash 峰谷计价，不联网） | <img src="assets/thumbs/04-settings.png" alt="设置项" width="480" /> |
