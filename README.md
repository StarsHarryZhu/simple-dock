...
- **数据来源**：优先由 node half 通过同源只读端点 `/dsh-simple-dock/api/steps` 读出该会话在 DSH 会话记录里的**完整逐步用量**（不受页面加载范围影响），面板标注「完整记录」；端点不可用时回退为页面当前可见的步，标注「当前可见步（估算）」，其缺口按已计价部分的平均单价补算，不整体套用最新价目
- 其他不在内置表中的模型不显示预估费用
