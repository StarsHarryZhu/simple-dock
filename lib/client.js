window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* 官方上下文指示器（ContextMeter）与本插件的坞在同一底栏行、且是我们的相邻
		   兄弟（slot 条目不额外包 DOM），所以用相邻兄弟选择器给它套上与本插件一致的
		   胶囊背景：跟随「底栏面板样式」（半透明 / 传统）与「面板玻璃」的模糊、磨砂
		   滑杆（--dsh-dstat-blur / --dsh-dstat-frost）。兜底选择器覆盖上游将来给条目
		   加 wrapper 的情形；只设外观，不动官方点击展开的明细面板，也不干预其 hover。 */
		.dsstat-root + *,
		div:has(> .dsstat-root) > *:last-child {
		  border-radius: 999px;
		  transition: background-color 120ms ease, border-color 120ms ease;
		}
		.dsstat-root:not(.dsstat-glass) + *,
		div:has(> .dsstat-root:not(.dsstat-glass)) > *:last-child {
		  background: var(--dsw-alias-bg-base);
		  border: 1px solid var(--dsw-alias-border-l1);
		}
		.dsstat-root.dsstat-glass + *,
		div:has(> .dsstat-root.dsstat-glass) > *:last-child {
		...
		body[data-ds-dark-theme] div:has(> .dsstat-root.dsstat-glass) > *:last-child {
