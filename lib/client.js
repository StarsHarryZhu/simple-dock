window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
		  ...
		  'cost.source.host': '完整记录',
		  'cost.source.session': '当前可见步（估算）',
		...
		const en = {
		  ...
		  'cost.source.host': 'full log',
		  'cost.source.session': 'visible steps (est.)',
		...
		async function fetchHostSteps(sessionId) {
		...
		async function computeSessionCost(nodes, usage, currencyCode, hostSteps) {
		...
		// 端点不可用时回退到页面节点路径。
		async function computeSessionCostFor(sessionId, nodes, usage, currencyCode) {
