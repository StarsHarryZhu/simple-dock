window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
		  ...
		  'mode.classic': '传统',
		  'pricing.label': '价格表',
		  'pricing.builtin': '内置 · v4-pro/flash 峰谷计价 · 更新于 2026-09-10（flash 新价）',
		...
		const en = {
		  ...
		  'pricing.builtin': 'Built-in · v4-pro/flash peak pricing · updated 2026-09-10 (flash revision)',
		...
		// prices.js — 内置价格模型（唯一来源，不拉取外部价格表）。
		// 计费原理：金额 = token 用量 × 模型价格表，本地计算。
		//   - 思维链按输出价计费（无独立 reasoning 价目）；
		//   - 未命中输入价已含缓存写入，不重复计费（无 cacheWrite 价目）。
		// node 半区（成本引擎）与 web client（回退计价）共用本模块，只此一份。
		
		/** 计价口径版本：价格表或计价规则变化时递增，带旧版本的成本记录随之作废。 */
		const PRICE_VERSION = 10
		...
		const FLASH_REVISION_START_MS = Date.UTC(2026, 8, 10, 4)
		// 北京时间 = UTC+8。
		...
		const getGlassColor = () => glassColorPref.get()
		const getSolidColor = () => solidColorPref.get()
		...
		function setGlassColor(v) {
		  glassColorPref.set(v)
		  applyGlassVars()
		}
		function setSolidColor(v) {
		...
		// ---- 成本端点（host 预算 + 增量维护） ----
		// node 半区在启动时就把每个会话的完整成本算好（没有成本记录的会话全量回填），
		// 运行期只按新步增量累加，并把每步成本写回会话记录。这里只读结果，不再逐点
		// 重算，因此数值不随页面加载范围漂移；端点不可用时回退到节点路径。
		const COST_ROUTE = '/dsh-simple-dock/api/cost'
		
		/** 取某会话由 host 维护的成本；不可用或失败返回 null（调用方回退节点路径）。 */
		async function fetchSessionCost(sessionId, refresh) {
		  if (typeof sessionId !== 'string' || sessionId === '') return null
		  if (typeof fetch !== 'function') return null
		  try {
		    const query = `sessionId=${encodeURIComponent(sessionId)}${refresh === true ? '&refresh=1' : ''}`
		    const response = await fetch(`${COST_ROUTE}?${query}`, { headers: {} })
		    if (!response.ok) return null
		    const body = await response.json()
		    if (body === null || typeof body !== 'object' || body.ok !== true) return null
		    return {
		      ok: true,
		      steps: num(body.steps) ?? 0,
		      totals: body.totals,
		      subagents: body.subagents,
		      unpriced: num(body.unpriced) ?? 0,
		...
		/**
		 * 成本读取入口：先用 host 结果（精确、稳定）；端点不可用时按当前币种走
		 * 页面节点计价（回退），保证没有 host 半区时面板仍显示估算。
		 */
		async function loadSessionCost(sessionId, nodes, usage, currency, refresh) {
		  const remote = await fetchSessionCost(sessionId, refresh)
		  if (remote !== null) return remote
		  const local = await computeSessionCost(nodes, usage, currency)
		  if (local.ok !== true) return { ok: false, error: local.error, source: 'local' }
		  return {
		    ok: true,
		    steps: null,
		    totals: { [local.currency]: local.cost },
		    source: 'local',
		  }
		}
		...
		// 回退计价：host 端点不可用时按页面节点逐点取价（{ ok, currency, model,
		// cost, skipped }）。nodes = 会话快照节点；usage = readUsage() 投影聚合
		// （兜底差额）。每步按其消耗时刻取峰/谷价累加。
		async function computeSessionCost(nodes, usage, currencyCode) {
		  ...
		  for (const row of rows) {
		    // 时间线包含没有 usage 的步：兜底要落在会话真实末尾时刻，而不是
		    // 浏览器当前时间（后者在调价/峰谷切换后会把历史用量算到新价上）。
		  ...
		  // 差额兜底：节点缺 usage 的步（或窗口外的用量）。优先按「已计价部分的
		  // 加权平均单价」补算——历史用量占大头时，用最近时刻的价格（可能已跨越
		  // 峰谷/调价边界）会把它们整体折算成最新价，调价后尤其会明显低估；
		  // 没有任何已计价样本（无价目模型/空节点）时才退回最近时刻价。
		  ...
		  if (leftover.uncached > 0 || leftover.out > 0 || leftover.read > 0 || leftover.write > 0) {
		    const pricedMiss = priced.uncached + priced.write
		    // 已计价部分的每 token 平均成本（金额 ÷ token）。
		    const avgHit = priced.read > 0 ? total.hit / priced.read : null
		    const avgMiss = pricedMiss > 0 ? total.miss / pricedMiss : null
		    const avgOut = priced.out > 0 ? total.out / priced.out : null
		    if (avgHit !== null || avgMiss !== null || avgOut !== null) {
		      const lastPrice = priceAt(fallbackModel, currencyCode, lastAt !== null ? lastAt : Date.now())
		      const unit = (avg, unitPrice) => avg !== null ? avg : (unitPrice || 0) / 1e6
		      const hit = leftover.read * unit(avgHit, lastPrice && lastPrice.cacheRead)
		      const miss = (leftover.uncached + leftover.write) * unit(avgMiss, lastPrice && lastPrice.input)
		      const out = leftover.out * unit(avgOut, lastPrice && lastPrice.output)
		      total.hit += hit
		      total.miss += miss
		      total.out += out
		      total.total += hit + miss + out
		...
		.dsstat-root {
		  ...
		  gap: 8px;
		  /* 与官方底栏同行：占据 dock 行的剩余宽度（左边步数、右边命中率），
		     官方上下文指示器紧跟在右侧；不再自己占满整行并居中。 */
		  flex: 1 1 auto;
		  width: auto;
		  max-width: none;
		  min-width: 0;
		...
		/* 上下文改为本插件自渲染（第三个按钮 + 面板，样式与另两个一致），因此把
		   官方 ContextMeter 隐藏，避免重复显示。锚点是 slot outlet 的
		   [data-slot="<key>"]（display:contents 的布局中性元素），ContextMeter 就渲染
		   在它之后；:has(.dsstat-root) 限定「只有我们确实接管这一行时才隐藏」——
		   hero / 无会话时本插件不渲染，官方指示器照常显示。 */
		[data-slot="conversation.composer.dock"]:has(.dsstat-root) + * {
		  display: none;
		}
		.dsstat-seg {
		  display: inline-flex;
		...
		.dsstat-panel {
		  ...
		  overflow: hidden;
		  /* 传统模式背景 = 用户在「设置 → 通用设置 → 背景色」里选的传统模式色，
		     未设置时回退到页面主背景（--dsw-alias-bg-base，深浅主题自动一致）；
		     半透明模式由内联 color-mix 填充覆盖，不受此值影响。 */
		  background: var(--dsh-dstat-solid-color, var(--dsw-alias-bg-base));
		...
		/* ---- 上下文：结构、尺寸与配色照官方 ContextMeter（圆环 + 占比条 + 图例），
		   只有面板背景/边框/阴影沿用我们自己的 .dsstat-panel（传统实底或半透明
		   玻璃，跟随背景色与玻璃滑杆设置）。放在 .dsstat-panel 之后以覆盖其尺寸。 ---- */
		.dsstat-right-group { display: inline-flex; align-items: center; gap: 6px; }
		.dsstat-ctx-trigger {
		  display: inline-flex;
		  align-items: center;
		  gap: 6px;
		...
		.dsstat-ctx-trigger:hover,
		.dsstat-ctx-trigger.open {
		  background: var(--dsw-alias-interactive-bg-hover);
		  color: var(--dsw-alias-label-secondary);
		}
		.dsstat-ctx-track { fill: none; stroke: var(--dsw-alias-border-l3); stroke-width: 2; }
		.dsstat-ctx-fill { fill: none; stroke: var(--dsw-alias-label-tertiary); stroke-width: 2; stroke-linecap: round; }
		.dsstat-ctx-panel {
		...
		.dsstat-ctx-header { display: flex; align-items: center; gap: 6px; }
		.dsstat-ctx-headline { color: var(--dsw-alias-label-tertiary); }
		.dsstat-ctx-headline:empty { display: none; }
		.dsstat-ctx-percent { font-weight: 500; color: var(--dsw-alias-label-primary); }
		.dsstat-ctx-figures {
		  margin-left: auto;
		  font-weight: 500;
		  font-variant-numeric: tabular-nums;
		...
		.dsstat-ctx-system { --meter-tint: var(--dsw-static-neutral-bluish-400); }
		/* 设计体系没有紫色静态 token，官方同样用 violet-400 字面量。 */
		.dsstat-ctx-tools { --meter-tint: rgb(167, 139, 250); }
		.dsstat-ctx-messages { --meter-tint: var(--dsw-static-blue-450); }
		.dsstat-ctx-rows { margin: 6px 0 0; }
		.dsstat-ctx-row {
		  display: flex;
		  align-items: center;
		  justify-content: space-between;
		...
		.dsstat-ctx-row dt { color: var(--dsw-alias-label-secondary); }
		.dsstat-ctx-row dd { margin: 0; font-variant-numeric: tabular-nums; color: var(--dsw-alias-label-primary); }
		.dsstat-ctx-swatch {
		...
		/* Settings → General: 底栏统计坞总开关（与插件卡片的开关同一个偏好）。
		   这一行在停用状态下依然渲染，否则关掉后就再也开不回来。 */
		.dsstat-enabled-row {
		  display: flex;
		  align-items: center;
		  justify-content: space-between;
		...
		.dsstat-enabled-text { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
		.dsstat-enabled-label { color: var(--dsw-alias-label-primary); }
		.dsstat-enabled-desc { font-size: 11px; color: var(--dsw-alias-label-tertiary); }
		/* Settings → General: 背景色（半透明 / 传统两个取色器 + 跟随主题）。 */
		.dsstat-color-row {
		  display: flex;
		  align-items: center;
		  justify-content: space-between;
		...
		.dsstat-color-items { display: inline-flex; align-items: center; gap: 12px; }
		.dsstat-color-item { display: inline-flex; align-items: center; gap: 6px; }
		/* 与设置页其它项一致的控件风格：小圆角色块 + 十六进制值文本（不用原生的大
		   方块，避免深色主题下显示成两块黑）。 */
		.dsstat-color-input {
		  width: 18px;
		  height: 18px;
		...
		.dsstat-color-input::-webkit-color-swatch-wrapper { padding: 0; }
		.dsstat-color-input::-webkit-color-swatch { border: none; border-radius: 5px; }
		.dsstat-color-input:hover { border-color: var(--dsw-alias-brand-primary); }
		.dsstat-color-value {
		  min-width: 62px;
		  font-size: 11px;
		  font-variant-numeric: tabular-nums;
		...
		/* Settings → Simple Dock：独立设置区（设置导航里单开一页）。 */
		.dsstat-section { display: flex; flex-direction: column; gap: 12px; font-family: var(--dsw-font-family); }
		.dsstat-section-head { display: flex; flex-direction: column; gap: 2px; }
		.dsstat-section-title { font-size: 15px; font-weight: 600; color: var(--dsw-alias-label-primary); }
		.dsstat-section-desc { font-size: 12px; line-height: 18px; color: var(--dsw-alias-label-tertiary); }
		.dsstat-section-body {
		  display: flex;
		...
		/* Plugins 页卡片下的一行状态描述（keyed bundle config 的 summary 视图）。 */
		.dsstat-card-summary {
		  color: var(--dsw-alias-label-tertiary);
		  font-size: 12px;
		  line-height: 18px;
