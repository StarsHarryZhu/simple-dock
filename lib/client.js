window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		.dsstat-pricing-text { font-size: 11px; color: var(--dsw-alias-label-tertiary); text-align: right; }
		.dsstat-mode-btn {
		  appearance: none;
		  border: 1px solid var(--dsw-alias-border-l1);
		  background: var(--dsw-alias-interactive-bg-hover);
		...
		/* 自绘色块：原生 <input type="color"> 的 swatch 在深色主题下会渲染成空块
		   （看起来是黑的），所以色块由我们自己画，input 透明覆盖在上面只负责弹出
		   系统取色器。 */
		.dsstat-color-field { position: relative; display: inline-flex; width: 18px; height: 18px; flex: none; }
		.dsstat-color-chip {
		  width: 100%;
		  height: 100%;
		  box-sizing: border-box;
		...
		.dsstat-color-field:hover .dsstat-color-chip { border-color: var(--dsw-alias-brand-primary); }
		.dsstat-color-value {
		  min-width: 62px;
		  font-size: 11px;
		  font-variant-numeric: tabular-nums;
