window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/* 官方上下文指示器（ContextMeter）在输入栏里就渲染在 conversation.composer.dock
		   这个 slot 之后，而框架给每个 slot 渲染点固定提供 [data-slot="<key>"] 锚点
		   （display:contents，布局中性）——所以用该锚点的相邻兄弟选择器给它套上与本插件
		   一致的胶囊背景：跟随「底栏面板样式」（半透明 / 传统）与「面板玻璃」的模糊、
		   磨砂滑杆。.dsstat-root + * 作为无锚点情形的兼容；只设外观，不动官方点击展开
		   的明细面板，也不干预其 hover。 */
		...
		[data-slot="conversation.composer.dock"]:not(:has(.dsstat-glass)) + *,
		.dsstat-root:not(.dsstat-glass) + * {
		  background: var(--dsw-alias-bg-base);
		  border: 1px solid var(--dsw-alias-border-l1);
		}
		[data-slot="conversation.composer.dock"]:has(.dsstat-glass) + *,
		.dsstat-root.dsstat-glass + * {
		  border: 1px solid #132d5342;
		  background-color: color-mix(in srgb, rgb(255 255 255) calc(50% * var(--dsh-dstat-frost, 1)), transparent);
		  background-image: linear-gradient(180deg, color-mix(in srgb, rgb(255 255 255) calc(35% * var(--dsh-dstat-frost, 1)), transparent) 0%, transparent 70%);
		  backdrop-filter: blur(var(--dsh-dstat-blur, 14px)) saturate(1.2);
		  -webkit-backdrop-filter: blur(var(--dsh-dstat-blur, 14px)) saturate(1.2);
		  box-shadow: inset 0 1px #ffffff80, 0 6px 20px #132d531a;
		}
		body[data-ds-dark-theme] [data-slot="conversation.composer.dock"]:has(.dsstat-glass) + *,
		body[data-ds-dark-theme] .dsstat-root.dsstat-glass + * {
		  border-color: #94b4dc52;
		  background-color: color-mix(in srgb, rgb(42 46 56) calc(50% * var(--dsh-dstat-frost, 1)), transparent);
		  background-image: linear-gradient(180deg, color-mix(in srgb, rgb(22 25 34) calc(50% * var(--dsh-dstat-frost, 1)), transparent) 0%, transparent 70%);
